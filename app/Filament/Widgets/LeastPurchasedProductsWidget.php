        ...
        return $table
            ->query(
                Product::query()
                    ...
                    ->orderByRaw("COALESCE((SELECT SUM(od.quantity) FROM order_details od JOIN orders o ON o.id = od.order_id WHERE od.product_id = products.id AND o.status != 'cancelled'), 0) ASC")
                    ->limit(5)
            )
            ->columns([
